...
The option `--override` can be used in subsequent runs:

`dotnet-combine single-file C:/MyAmazingProject --override --output C:/tmp/amazing.cs`

If you prefer to keep different versions of your output file, but would like to identify them, you can use `--prefix` and `--suffix` together with your output dir in `--output`:
...
- Extracting all  files from a .NET project.

- Extracting all your source code files (`.sln`, `.csproj`, `.cs`, `.razor`, `.xaml`, etc.) in a GitHub Action step to be able to upload them as artifact.
...
If you want to create a `.zip` with all C++ source code in your `MyGeekyProject` project, located in `/home/ed/GeekyProject/`, you can simply do:
...
If you don't use `--override` (abbreviated as `-f`), the tool will prevent you from overriding the generated file. You can enable it by doing:

`dotnet-combine zip /home/ed/GeekyProject -f --extensions ".cpp;cxx:cc;.h;.hpp" --output /home/ed/GeekyProject/artifacts/src_v1`

However, you may prefer to keep different versions of your output file without having to worry about specifying different `_vN` in the name. You can achieve that by only including a dir path in `--output`, and making use of `--prefix` and/or `--suffix`:
...
If you want to pack your `MyGeekyProject` source files as part of your GitHub Actions CI pipeline, you can do it as it follows:
...
If you experience difficulties using `dotnet-combine`, please [open an issue](https://github.com/eduherminio/dotnet-combine/issues/new/choose) detailing what you want to achieve and/or the command you're running.

Feature requests are welcome.

PRs are also more than welcome, but feel free to open an issue before carrying out any major changes to better align on them.
